...
export default function YpohrewseisPage() {
  ...
  const [form, setForm] = useState({ name: '', obligation_type: 'fixed', amount: '', employee_name: '', recurrence: 'monthly', next_due_date: '' })
  ...
  async function handleSave() {
    if (!form.name && !form.employee_name) { toast.error('Συμπλήρωσε όνομα'); return }
    const payload: any = {
      name: form.obligation_type === 'employee' ? form.employee_name : form.name,
    ...
    setShowForm(false)
    setForm({ name: '', obligation_type: 'fixed', amount: '', employee_name: '', recurrence: 'monthly', next_due_date: '' })
    loadData()
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between px-6 py-4 bg-white border-b border-gray-100 sticky top-0 z-10">
        ...
        <button className="btn-primary btn-sm" onClick={() => setShowForm(true)}>+ Νέα υποχρέωση</button>
      </div>
              ...
              <tbody>
                ...
                : filtered.map(o=>(
                  <tr key={o.id} className="hover:bg-gray-50">
                    ...
                    <td className="table-cell"><button onClick={()=>handleDelete(o.id)} className="text-gray-300 hover:text-red-500 text-lg leading-none">×</button></td>
                  </tr>
                ))}
          ...
          <div className="space-y-4">
            ...
            <div><label className="label">{form.obligation_type==='employee'?'Ονοματεπώνυμο υπαλλήλου':'Όνομα υποχρέωσης'}</label>
              <input className="input" value={form.obligation_type==='employee'?form.employee_name:form.name}
                onChange={e=>setForm(f=>form.obligation_type==='employee'?{...f,employee_name:e.target.value,name:e.target.value}:{...f,name:e.target.value})} placeholder="..." />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div><label className="label">Ποσό (€) — 0 = κατά πληρωμή</label><input type="number" className="input" value={form.amount} onChange={e=>setForm(f=>({...f,amount:e.target.value}))} placeholder="0.00" step="0.01" /></div>
            ...
            {form.obligation_type==='periodic' && (
              <div><label className="label">Επόμενη ημ. λήξης</label><input type="date" className="input" value={form.next_due_date} onChange={e=>setForm(f=>({...f,next_due_date:e.target.value}))} /></div>
            )}
