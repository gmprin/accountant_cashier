...
export default function DashboardPage() {
  ...
  return (
    <div>
      ...
      <div className="p-6 space-y-6">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            <div className="metric-card">
              ...
              <p className="text-lg font-semibold text-green-600">{formatMoney(weekly?.total_receipts||0)}</p>
            </div>
            <div className="metric-card">
              <p className="text-xs text-gray-500 mb-1">Πληρωμές</p>
              <p className="text-lg font-semibold text-red-600">{formatMoney((weekly?.total_expenses||0)+(weekly?.total_salaries||0))}</p>
            </div>
            <div className="metric-card">
              <p className="text-xs text-gray-500 mb-1">Διανομές κερδών</p>
              <p className="text-lg font-semibold text-amber-600">{formatMoney(weekly?.total_distributions||0)}</p>
            </div>
            <div className="metric-card">
              <p className="text-xs text-gray-500 mb-1">Αποτέλεσμα</p>
              <p className={`text-lg font-semibold ${(weekly?.net_result||0)>=0?'text-green-600':'text-red-600'}`}>{formatMoney(weekly?.net_result||0)}</p>
            </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            <div className="metric-card">
              <p className="text-xs text-gray-500 mb-1">Εισπράξεις</p>
              <p className="text-base font-semibold text-green-600">{formatMoney(monthly?.total_receipts||0)}</p>
            </div>
            <div className="metric-card">
              <p className="text-xs text-gray-500 mb-1">Πληρωμές</p>
              <p className="text-base font-semibold text-red-600">{formatMoney((monthly?.total_expenses_fixed||0)+(monthly?.total_expenses_periodic||0)+(monthly?.total_expenses_adhoc||0)+(monthly?.total_salaries||0))}</p>
            </div>
            <div className="metric-card">
              <p className="text-xs text-gray-500 mb-1">Διανομές κερδών</p>
              <p className="text-base font-semibold text-amber-600">{formatMoney(monthly?.total_distributions||0)}</p>
            </div>
            <div className="metric-card">
              <p className="text-xs text-gray-500 mb-1">Αποτέλεσμα μήνα</p>
              <p className={`text-base font-semibold ${(monthly?.net_result||0)>=0?'text-green-600':'text-red-600'}`}>{formatMoney(monthly?.net_result||0)}</p>
            </div>
                  ...
                  <table className="w-full">
                    ...
                    <tbody>
                      ...
                      {allReceipts.length===0 && (
                        <tr><td colSpan={3} className="table-cell text-center text-gray-400 py-6">Καμία είσπραξη ακόμα</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
                <div className="px-5 py-2 border-t border-gray-100 flex justify-between text-sm bg-gray-50">
                  <span className="text-gray-500">Σύνολο ({allReceipts.length} κινήσεις)</span>
                  <span className="font-semibold text-green-600">+{formatMoney(allReceipts.reduce((s,e)=>s+e.amount,0))}</span>
                </div>
              </>
            )}
          </div>
                  ...
                  <table className="w-full">
                    ...
                    <tbody>
                      ...
                      {allPayments.length===0 && (
                        <tr><td colSpan={3} className="table-cell text-center text-gray-400 py-6">Καμία πληρωμή ακόμα</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
                <div className="px-5 py-2 border-t border-gray-100 flex justify-between text-sm bg-gray-50">
                  <span className="text-gray-500">Σύνολο ({allPayments.length} κινήσεις)</span>
                  <span className="font-semibold text-red-600">−{formatMoney(allPayments.reduce((s,e)=>s+e.amount,0))}</span>
                </div>
              </>
            )}
          </div>
      ...
      {/* Modal υποχρεώσεων */}
      {(showOblWeekModal || showOblMonthModal) && (
        <Modal
          title={showOblWeekModal ? 'Ανεξόφλητες υποχρεώσεις — Εβδομάδα' : 'Ανεξόφλητες υποχρεώσεις — Μήνας'}
          onClose={()=>{ setShowOblWeekModal(false); setShowOblMonthModal(false) }}
        >
                    ...
                    {oblData.map(o=>(
                      <tr key={o.id} className="hover:bg-gray-50">
                        ...
                        <td className="table-cell text-xs">
                          {o.next_due_date
                            ? <span className={new Date(o.next_due_date)<new Date()?'text-red-600 font-medium':'text-gray-500'}>{formatDate(o.next_due_date)}</span>
                            : '—'
                          }
                        </td>
                      </tr>
